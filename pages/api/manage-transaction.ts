...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === 'PATCH') {
    ...
    const allowedFields = ['ticketType', 'price', 'quantity', 'total', 'paymentMethod', 'uid', 'cashier']
    ...
    try {
      ...
      return res.status(500).json({ error: err.message || 'Gagal mengupdate transaksi' })
    }
  }
  ...
  if (req.method === 'DELETE') {
    try {
      ...
      return res.status(500).json({ error: err.message || 'Gagal menghapus transaksi' })
    }
  }
